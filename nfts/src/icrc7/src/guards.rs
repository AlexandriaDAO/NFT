use crate::{
    ...
    types::{
        icrc37::{
            ...
            RevokeTokenApprovalError, TransferFromArg, TransferFromError,
        },
        icrc7::TransferError,
    },
    TransferArg, ANONYMOUS,
};
...
impl Validate for TransferArg {
    ...
    fn validate(
        ...
        if let Some(created_at_time) = self.created_at_time {
            if created_at_time > now + settings.permitted_drift * 10_000_000_000 {
                return Err(TransferError::CreatedInFuture {
                    ledger_time: now + settings.permitted_drift,
                });
            }
            if created_at_time
                < now - (settings.tx_window + settings.permitted_drift) * 10_000_000_000
            {
...
impl Validate for ApproveTokenArg {
    ...
    fn validate(
        ...
        if let Some(expires_at) = self.approval_info.expires_at {
            if expires_at < now + settings.permitted_drift * 10_000_000_000 {
                return Err(ApproveTokenError::GenericError {
                    error_code: Nat::from(0u64),
